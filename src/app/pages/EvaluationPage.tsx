...
import { UploadCloud, AlertTriangle, CheckCircle, Info, X } from "lucide-react";
...
function FormField({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-1.5">
      <label style={{ fontSize: 11, color: C.textSoft, letterSpacing: "0.4px", textTransform: "uppercase", fontFamily: "'Lora', serif" }}>
        {label}
      </label>
      {children}
    </div>
...
function AnalysisStep({ label, delay }: { label: string; delay: number }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span style={{ fontSize: 13, color: done ? C.text : C.textSoft, fontFamily: "'Lora', serif", transition: "color 0.3s" }}>{label}</span>
    </div>
  );
}
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div className="rounded-2xl p-8 flex flex-col gap-5" style={{ background: C.white, border: "0.5px solid rgba(139,58,15,0.1)" }}>
        ...
        >
          <div style={{ fontSize: 12, color: C.earth, fontFamily: "'Lora', serif", display: "flex", alignItems: "center", gap: 6 }}>
        ...
        {showAdvanced && (
          <div className="flex flex-col gap-4" style={{ padding: "1rem", background: C.ocrePale, borderRadius: 12, border: "0.5px solid rgba(139,58,15,0.12)" }}>
            <div style={{ fontSize: 11, color: C.textSoft, fontFamily: "'Lora', serif", lineHeight: 1.6 }}>
        ...
        <button
          ...
          style={{ background: !age || loading ? "rgba(194,87,115,0.35)" : C.rose, color: C.white, border: "none", fontFamily: "'Lora', serif", fontSize: 15, cursor: !age || loading ? "not-allowed" : "pointer" }}
          onMouseEnter={(e) => { if (age && !loading) (e.currentTarget as HTMLElement).style.background = C.roseDark; }}
          onMouseLeave={(e) => { if (age && !loading) (e.currentTarget as HTMLElement).style.background = C.rose; }}
        >
      ...
      <div className="flex flex-col gap-5">
        ...
        {error && !loading && (
          <div className="rounded-2xl p-6" style={{ background: "#FFF0F0", border: "0.5px solid rgba(194,87,115,0.3)" }}>
            <p style={{ fontSize: 13, color: C.roseDark, fontFamily: "'Lora', serif", lineHeight: 1.7 }}>⚠️ {error}</p>
          </div>
        )}
            ...
            <div className="rounded-2xl p-8 flex flex-col gap-4" style={{ background: riskConfig[result.level].bg }}>
              ...
              <div>
                <div className="flex justify-between mb-2">
                  <span style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", fontFamily: "'Lora', serif" }}>Probabilité de malignité</span>
                  <span style={{ fontSize: 13, color: C.gold, fontFamily: "'Playfair Display', serif", fontWeight: 600 }}>{result.pct}%</span>
                </div>
                <div style={{ height: 8, background: "rgba(255,255,255,0.15)", borderRadius: 10, overflow: "hidden" }}>
                  <div style={{ height: "100%", width: `${result.pct}%`, background: C.gold, borderRadius: 10, transition: "width 1.2s ease" }} />
                </div>
              </div>
              <div className="rounded-xl px-4 py-3" style={{ background: "rgba(0,0,0,0.15)" }}>
                <div style={{ fontSize: 10, color: "rgba(255,255,255,0.5)", textTransform: "uppercase", letterSpacing: "1px", marginBottom: 4, fontFamily: "'Lora', serif" }}>
                  Classification IA
                </div>
              ...
              <div className="rounded-xl px-4 py-3" style={{ background: "rgba(0,0,0,0.12)" }}>
                ...
                <p style={{ fontSize: 13, color: "rgba(255,255,255,0.85)", lineHeight: 1.7, fontFamily: "'Lora', serif" }}>{result.advice}</p>
              </div>
            </div>

            <div className="rounded-xl px-5 py-4" style={{ background: C.sandDeep, border: "0.5px solid rgba(139,58,15,0.1)" }}>
              <p style={{ fontSize: 12, color: C.textSoft, lineHeight: 1.7, fontFamily: "'Lora', serif" }}>
                ⚠️ <em>Cet outil est une aide à la décision, non un diagnostic médical. Consultez toujours un professionnel de santé qualifié.</em>
              </p>
            </div>
          </>
        )}
        ...
        {!result && !loading && !error && (
          <div className="rounded-2xl p-10 flex flex-col items-center justify-center text-center gap-4" style={{ background: C.white, border: "0.5px dashed rgba(139,58,15,0.2)", minHeight: 280 }}>
            ...
            <p style={{ fontSize: 13, color: C.textSoft, fontFamily: "'Lora', serif", maxWidth: 260, lineHeight: 1.7 }}>Remplissez le formulaire et cliquez sur "Lancer l'évaluation".</p>
          </div>
        )}

        <div className="grid grid-cols-3 gap-3">
          {[{ emoji: "🕐", text: "Résultat en moins de 30s" }, { emoji: "🔒", text: "Données non enregistrées" }, { emoji: "🆓", text: "Entièrement gratuit" }].map((tip) => (
            <div key={tip.text} className="rounded-xl p-3 text-center" style={{ background: C.sand, border: "0.5px solid rgba(139,58,15,0.08)" }}>
              <div style={{ fontSize: 20, marginBottom: 4 }}>{tip.emoji}</div>
              <div style={{ fontSize: 11, color: C.textMid, fontFamily: "'Lora', serif", lineHeight: 1.4 }}>{tip.text}</div>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div className="flex flex-col gap-4">
        {step === "idle" && (
          ...
          >
            ...
            <div>
              ...
              <p style={{ fontSize: 13, color: C.textSoft, fontFamily: "'Lora', serif", lineHeight: 1.7 }}>
                Formats acceptés : JPG, PNG<br />Mammographies, échographies, radiographies
              </p>
            </div>
            <span className="inline-block rounded-full px-6 py-2.5" style={{ background: C.sand, border: "0.5px solid rgba(139,58,15,0.2)", fontSize: 13, color: C.earth, fontFamily: "'Lora', serif" }}>
        ...
        {(step === "preview" || step === "analyzing" || step === "result") && previewUrl && (
          <div className="flex flex-col gap-3">
            <div className="flex items-center justify-between">
              <span style={{ fontSize: 12, color: C.textSoft, fontFamily: "'Lora', serif" }}>Image chargée</span>
              <button onClick={reset} className="flex items-center gap-1.5 rounded-full px-3 py-1.5" style={{ background: C.sand, border: "none", cursor: "pointer", fontSize: 12, color: C.earth, fontFamily: "'Lora', serif" }}>
              ...
              {step === "analyzing" && (
                <div className="absolute inset-0 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.45)" }}>
                  <div className="rounded-xl px-6 py-4 text-center" style={{ background: "rgba(28,43,26,0.95)", border: "0.5px solid rgba(201,151,58,0.3)" }}>
                    <div style={{ fontSize: 11, color: C.gold, letterSpacing: "1.5px", textTransform: "uppercase", fontFamily: "'Lora', serif", marginBottom: 6 }}>Analyse en cours</div>
        ...
        {error && (
          <div className="rounded-xl px-5 py-4" style={{ background: "#FFF0F0", border: "0.5px solid rgba(194,87,115,0.3)" }}>
            <p style={{ fontSize: 13, color: C.roseDark, fontFamily: "'Lora', serif" }}>⚠️ {error}</p>
          </div>
        )}

        {step === "preview" && (
          <button onClick={startAnalysis} className="w-full rounded-full py-3.5 transition-all duration-200"
            style={{ background: C.rose, color: C.white, border: "none", fontFamily: "'Lora', serif", fontSize: 15, cursor: "pointer" }}
            ...
            {[
              ...
              <div key={s.title} className="flex gap-4 items-start">
                ...
                <div>
                  ...
                  <div style={{ fontSize: 13, color: C.textSoft, fontFamily: "'Lora', serif", lineHeight: 1.65 }}>{s.text}</div>
                </div>
              </div>
            ))}
            <div className="rounded-xl px-4 py-3" style={{ background: C.ocrePale, border: "0.5px solid rgba(212,114,26,0.2)" }}>
              <p style={{ fontSize: 12, color: C.textMid, fontFamily: "'Lora', serif", lineHeight: 1.7 }}>
          ...
          <div className="rounded-2xl p-8 flex flex-col gap-4" style={{ background: C.white, border: "0.5px solid rgba(139,58,15,0.1)" }}>
            ...
            <div>
              <div className="flex justify-between mb-1.5">
                <span style={{ fontSize: 12, color: C.textSoft, fontFamily: "'Lora', serif" }}>Progression</span>
                <span style={{ fontSize: 12, color: C.earth, fontFamily: "'Lora', serif", fontWeight: 500 }}>{analysisProgress}%</span>
              </div>
              ...
              {analysisSteps.map((s, i) => (
                <div key={s} className="flex items-center gap-3">
                  <div style={{ width: 20, height: 20, borderRadius: "50%", background: i < currentStepIdx ? C.forestMid : i === currentStepIdx ? C.ocre : "rgba(139,58,15,0.1)", border: i < currentStepIdx || i === currentStepIdx ? "none" : "1.5px solid rgba(139,58,15,0.2)", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", transition: "background 0.3s" }}>
                    {i < currentStepIdx && <span style={{ color: "#fff", fontSize: 10 }}>✓</span>}
                    {i === currentStepIdx && <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#fff", display: "block" }} />}
                  </div>
                  <span style={{ fontSize: 13, color: i < currentStepIdx ? C.text : i === currentStepIdx ? C.ocre : C.textSoft, fontFamily: "'Lora', serif", transition: "color 0.3s" }}>{s}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {step === "result" && result && (
          <>
            <div className="rounded-2xl p-8 flex flex-col gap-4" style={{ background: result.color }}>
              ...
              <div>
                <div className="flex justify-between mb-2">
                  <span style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", fontFamily: "'Lora', serif" }}>Score de suspicion</span>
              ...
              <div className="rounded-xl px-4 py-3" style={{ background: "rgba(0,0,0,0.15)" }}>
                <div style={{ fontSize: 10, color: "rgba(255,255,255,0.5)", textTransform: "uppercase", letterSpacing: "1px", marginBottom: 4, fontFamily: "'Lora', serif" }}>Classification IA</div>
                <p style={{ fontSize: 14, color: C.white, fontFamily: "'Playfair Display', serif", fontWeight: 600 }}>
                  {result.prediction === "Malignant" ? "Tumeur maligne détectée" : "Tumeur bénigne"} — {Math.round(result.confidence * 100)}% de confiance
                </p>
              </div>
              <div className="rounded-xl px-4 py-3" style={{ background: "rgba(0,0,0,0.12)" }}>
                <div style={{ fontSize: 10, color: "rgba(255,255,255,0.5)", textTransform: "uppercase", letterSpacing: "1px", marginBottom: 4, fontFamily: "'Lora', serif" }}>Recommandation</div>
                <p style={{ fontSize: 13, color: "rgba(255,255,255,0.85)", lineHeight: 1.7, fontFamily: "'Lora', serif" }}>{result.advice}</p>
              </div>
            </div>
            <div className="rounded-xl px-5 py-4" style={{ background: C.sandDeep, border: "0.5px solid rgba(139,58,15,0.1)" }}>
              <p style={{ fontSize: 12, color: C.textSoft, lineHeight: 1.7, fontFamily: "'Lora', serif" }}>
                ⚠️ <em>Cet outil est une aide à la décision. Les résultats ne constituent pas un diagnostic médical. Consultez impérativement un radiologue ou médecin.</em>
              </p>
            </div>
          </>
        )}
  ...
  return (
    <main style={{ background: C.sand, minHeight: "100vh" }}>
      <div style={{ background: C.forest, padding: "4rem 4rem 0" }}>
        <div style={{ fontSize: 11, color: C.gold, letterSpacing: "2px", textTransform: "uppercase", fontFamily: "'Lora', serif", marginBottom: "0.8rem" }}>
          Outil d'évaluation
        </div>
        ...
        <div className="flex gap-0">
          {([
            { key: "clinique" as Tab, label: "🧬 Données cliniques", desc: "Formulaire médical + IA" },
            { key: "image"    as Tab, label: "🩻 Image radiographique", desc: "Upload & analyse IA" },
          ] as const).map((tab) => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)} className="flex flex-col items-start px-6 py-4 transition-all duration-200"
              ...
              <span style={{ fontFamily: "'Lora', serif", fontSize: 14, color: activeTab === tab.key ? C.text : "rgba(255,255,255,0.6)", fontWeight: activeTab === tab.key ? 500 : 400 }}>{tab.label}</span>
              <span style={{ fontSize: 11, color: activeTab === tab.key ? C.textSoft : "rgba(255,255,255,0.35)", fontFamily: "'Lora', serif" }}>{tab.desc}</span>
            </button>
          ))}
        </div>
      </div>
